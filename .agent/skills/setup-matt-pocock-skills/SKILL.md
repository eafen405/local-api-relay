...
- repo root 的 `AGENTS.md` - 是否存在？其中是否已有 `## Agent skills` 或 `## 决策协作` section？
...
- 要添加到 repo root `AGENTS.md` 的 `## Agent skills` 与 `## 决策协作` blocks
...
- 文件不存在时创建。
- 文件已有内容时，只更新本 skill 管理的命名区块，不覆盖周围的用户内容。
- `CLAUDE.md` 可以继续保留项目已有指令；本 skill 不创建也不编辑它。

如果 `AGENTS.md` 已有 `## Agent skills` 或 `## 决策协作` block，就原地更新各自内容，而不是追加重复 block。不要覆盖周围 sections 的用户编辑。
...
遇到会改变项目长期形状的决策时，先建立相关证据：识别当前主导约束、不可违反的约束，以及能够验证该选择的最小可逆切入点。

在以下情况决定前暂停，并以结构化选项征求用户选择：
...
给出一个推荐项，以及一到两个确有实质差异的备选项。对每项只说明其长期影响，等待用户明确选择后再继续。上述范围之外存在不确定性时，优先采用最小、可逆的方案，并在交付摘要中说明取舍。

用户选定上述关键决策后，自动向 `~/.kimi-code/learning/projects/<项目目录名>-<项目根规范化绝对路径的 SHA-256 前 8 位>.md` 追加最小事实快照。先解析项目根的规范化绝对路径，计算其 SHA-256 并取前 8 位；不存在时创建父目录和日志文件。快照包含日期、任务、决策问题、推荐与备选、用户选择，以及用户明确表达的目标或理由。

该学习日志位于项目外，不属于正常项目上下文；除非用户显式调用 `/review-decisions`，否则不得读取它或基于它改变工程工作。
```
